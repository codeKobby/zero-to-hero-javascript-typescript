...
[← Previous lesson](../03_day_data_types/03_day_data_types.md) · [README](../README.md) · [Setup](../VS_CODE_SETUP.md) · [Day index](../DAY_INDEX.md) · [Next lesson →](../05_day_control_flow/05_day_control_flow.md)
...
The first runnable example introduces **Operators — Math, Truth, and Comparison**. Run it unchanged before editing it. Then read it line by line and write down what value exists after each declaration, which condition is tested, and what appears in the console.

| Line | Code | What the runtime is doing |
| ---: | --- | --- |
| 1 | `const formTotal = '5' + 5   // '55' — the string trap from Day 3` | Declaration or assignment: the runtime creates or updates a named value. |
| 2 | `const count = 0` | Declaration or assignment: the runtime creates or updates a named value. |
| 3 | `if (count === false) { /* ... */ } // this never runs — and the why is invisible` | Control-flow statement: the runtime decides whether or how this block runs. |

The table is a starting point, not a substitute for running the example. Change one value only, predict the output, run it, and explain the difference.

## Prediction experiment

Before changing the example, write a prediction. Test one normal input, one empty or missing input, and one boundary input relevant to **Operators — Math, Truth, and Comparison**. Record the input, your prediction, the observed output or error, and the rule you learned. Keep the failed prediction; it shows which mental model needs repair.

## Broken example and repair

Make one controlled mistake related to **Operators — Math, Truth, and Comparison**: misspell a name, use the wrong type, omit a return, call a function too early, or change one condition. Run it and capture the useful error or incorrect output. Explain the assumption that failed, then make the smallest repair and rerun the normal and boundary cases. Do not hide the error with a broad catch or delete the failing experiment.
